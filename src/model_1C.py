...
class Model:
	...
	def __init__(self, arg1=None, arg2 = None, npar = 0):
		...
		if filename is not None:
			# Initialize with a .mod file
			if filename[-4:] == ".mod":
				data = np.loadtxt(filename, skiprows=1, dtype=np.float64).transpose()

				self.nx = 1
				self.ny = 1
				self.npar = len(data[0])
				self.log_tau = np.expand_dims(np.expand_dims(data[0],0),0)
				self.T = np.expand_dims(np.expand_dims(data[1],0),0)
				self.Pe = np.expand_dims(np.expand_dims(data[2],0),0)
				self.vmicro = np.expand_dims(np.expand_dims(data[3],0),0)
				self.B = np.expand_dims(np.expand_dims(data[4],0),0)
				self.vlos = np.expand_dims(np.expand_dims(data[5],0),0)/1e5
				self.gamma = np.expand_dims(np.expand_dims(data[6],0),0)
				self.phi = np.expand_dims(np.expand_dims(data[7],0),0)
				if len(data) > 8:
					self.z = np.expand_dims(np.expand_dims(data[8],0),0)
					self.Pg = np.expand_dims(np.expand_dims(data[9],0),0)
					self.rho = np.expand_dims(np.expand_dims(data[10],0),0)
				else:
					self.z = np.zeros((1,1,len(data[0])))
					self.Pg = np.zeros((1,1,len(data[0])))
					self.rho = np.zeros((1,1,len(data[0])))
				self.load = True
	...
	def correct_phi(self):
		"""
		SIR can give you any value for the azimuth, e.g. -250 deg, and therefore, to get rid of the ambiguity
		those values should be corrected so that I have values from 0 to 180 degrees.
	...
	def cut_data(self, x : int,nx : int,y : int,ny : int):
		"""
		This function cuts the data to a subset.
		...
		if(x+nx > self.nx):
			print("[cut_data] Out of range in x.")
		if(y+ny > self.ny):
			print("[cut_data] Out of range in y.")

		self.nx = nx
		self.ny = ny

		self.log_tau = self.log_tau[x:x+nx, y:y+ny, :]
		self.T = self.T[x:x+nx, y:y+ny, :]
		self.Pe = self.Pe[x:x+nx, y:y+ny, :]
		self.vmicro = self.vmicro[x:x+nx, y:y+ny, :]
		self.B = self.B[x:x+nx, y:y+ny, :]
		self.vlos = self.vlos[x:x+nx, y:y+ny, :]
		self.gamma = self.gamma[x:x+nx, y:y+ny, :]
		self.phi = self.phi [x:x+nx, y:y+ny, :]
		self.z = self.z[x:x+nx, y:y+ny, :]
		self.Pg = self.Pg[x:x+nx, y:y+ny, :]
		self.rho = self.rho[x:x+nx, y:y+ny, :]
	...
	def read_file(self, filename, x, y):
		"""
		Reads a .mod file
		...
		x : int
			at what x position the model is saved
		y : int
			at what y position the model is saved
	...
	def save(self, filename):
		...
		if np.mean(self.vlos) > 1e4:
			...
			print("       If this is not wished, look at the function save in model.py.")
...
class Error(Model):
	...
	def save(self, filename):
		...
		if (self.rho == np.zeros(shape=self.log_tau.shape)).all(): # No information in z, rho and Pg and thereofre do not save it
			models = np.stack([self.log_tau, self.T, self.Pe, self.vmicro, self.B, self.vlos*1.0e5, self.gamma,
									self.phi], axis=2, dtype=np.float64)
		else:
			models = np.stack([self.log_tau, self.T, self.Pe, self.vmicro, self.B, self.vlos*1.0e5, self.gamma,
									self.phi, self.z, self.Pg, self.rho], axis=2, dtype=np.float64)
