...
def help():
	print("visualizer - Visualizes the spectra and model parameters T, B, vlos and gamma.")
	print("Usage: python visualizer [OPTION]")
	print()
	print("1:		   Config")
	print("2:		   wavelength at which stuff is plotted in A or log tau value if a phys. model is plotted as the map")
	print()
	...
	print("-models:   Rel. path to the Models of the inversion if standard labelling is not used.")
	...
	print("-inc:      Plot inclination in deg")
	print("-azi:      Plot azimuth in deg")
	...
	print("-wave_V:   Plot Stokes V in another wavelength position in A")
...
def on_click(event, ll, I_obs, Q_obs, U_obs, V_obs, I_fit, Q_fit, U_fit, V_fit, Models1, Models2, Map):
	"""
...
def vlos_normalise(conf, models1_inv, models2_inv, stokes, wave_ind, index = 5):
	"""
	...
	models1_inv : numpy array
		Array containing all the models 1 from the inversion
	models2_inv : numpy array
		Array containing all the models 2 from the inversion
	stokes : numpy array
	...
	Return
	------
	numpy array with all the models 1 but vlos is normalised to the quiet sun
	numpy array with all the models 2 but vlos is normalised to the quiet sun
	"""
	...
	if len(quiet) > 1 and quiet[0] >= Map[0] and quiet[1] <= Map[1] and quiet[2] >= Map[2] and quiet[3] <= Map[3]:
		models1_inv[:,:,index]  = models1_inv[:,:,index] - np.mean(models1_inv[quiet[0]:quiet[1],quiet[2]:quiet[3],5])
		models2_inv[:,:,index]  = models2_inv[:,:,index] - np.mean(models2_inv[quiet[0]:quiet[1],quiet[2]:quiet[3],5])
	# Chosen range for quiet sun is not part of the inversion: Ask for the normalisation range:
		...
		plt.imshow(stokes[:,:,0,wave_ind].transpose(), origin = d.origin, vmin = 0.5, vmax = 1.0)
		plt.title("Choose range for normalisation of vlos")
		plt.show()
		quiet = (input ("Give a range to normalise vlos as a list: ")).replace(" ","").split(",")
		models1_inv[:,:,index]  = models1_inv[:,:,index] - np.mean(models1_inv[int(quiet[0])-Map[0]:int(quiet[1])-Map[0],int(quiet[2])-Map[2]:int(quiet[3])-Map[2],5])
		models2_inv[:,:,index]  = models2_inv[:,:,index] - np.mean(models2_inv[int(quiet[0])-Map[0]:int(quiet[1])-Map[0],int(quiet[2])-Map[2]:int(quiet[3])-Map[2],5])

	return models1_inv, models1_inv
...
def visualiser(conf, wave):
	...
	if "-norm" in sys.argv:
		models1_inv, models2_inv = vlos_normalise(conf, models1_inv, models2_inv, stokes_inv, range_wave_pix[0][0], 5)
