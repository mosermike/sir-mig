...
def help():
	...
	print("-title:    Title in plot")
	print("-num:      determines which range is used (1 = 1st line in grid, etc.) if not the first one is wanted.")
...
def Ic_test(conf, num):
	...
	############################
	ax.plot(stokes[Map[0]:Map[1]+1,Map[2]:Map[3]+1,0,wave_ind].flatten(),stokes_inv[:,:,0,wave_ind].flatten(), '.', label="")
	xlim = ax.get_xlim()
	ylim = ax.get_ylim()
